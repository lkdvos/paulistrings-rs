...
- **Determinism policy (user directive, v0.5; broadened 2026-08-31).** Bitwise output preservation is **not required — anywhere, for anything**. The correctness bar for engine changes is agreement to floating-point tolerance (`assert_terms_close`); equal-key summation order is unspecified and free to change between versions, configurations, and optimizations. The v0.5 wording that kept thread-count and repeat-run determinism "bitwise and load-bearing" is superseded: those tests, the fingerprint net (`layer_fingerprints_are_stable`), and any other byte-exact assertion are convenience tripwires, not policy — when one blocks a change that is correct to tolerance, regenerate its literals or demote it to `assert_terms_close` in the same commit, with a one-line note. Do not design, constrain, or reject optimizations to keep output bits stable.
...
**v0.6 (G1d: coefficient-only id materialization) has landed.** Plan in `research/plans/2026-08-31-v0.6-gather-merge.md`, measured results and two recorded negative results in `research/notes/2026-08-31-v0.6-results.md`. What it means for anyone touching the code:

- **Dense vs sparse identity streams (G1d).** `DeltaPlan::Local` classifies the identity delta as **dense** (its amplitude nonzero on every active support pattern — rotations, `GeneralUnitary1Q/2Q`, `AmplitudeDamping`) or sparse (Cliffords: CNOT 4/16, H 2/4). Dense: the id stream is 1:1 with the source bucket, so the gather materializes only the 16-byte coefficient into `GatherRun::id_coeff` and the merge borrows the key columns from `old[j]` in place (`id_x`/`id_z` stay empty); wide rotations are dense by construction. Sparse: unchanged v0.5 pre-filtered key+coeff materialization. Output is bitwise-unchanged from v0.5; the classification is pinned by `identity_density_classification`.
- **Measured (interleaved A/B, 3 pairs, ccqlin038; consistent direction across all pairs):** rotation_zz 1t −10.8%, trotter 1t −8.3%, gu2q −4.5/−9.1/−3.4% at 1/16/32t, cnot 1t −3.6%; high-t cells flat (no consistent change). The win is merge −12–15% + gather −4–8% busy — traffic/working-set, not a memory-wall breakthrough; trotter 32t stays ~9.2 ms/layer near the bandwidth ceiling.
- **Negative results, recorded (do not re-attempt without new data):** (1) *recompute-in-merge borrow* (the plan's original G1, plus a density hybrid and a split-loop variant): the per-row transform relocated into the merge's serially-dependent compare loop costs as much as the copy it saves (+~30% merge busy on the rotation family), and borrowing a sparse id stream scans 2–4× the rows (cnot +15–30%); (2) *M1 segment-copy merge* (gallop + bulk id segments): +20–35% merge busy — real id/rest densities make the average segment 1–2 rows. Both in the v0.6 note; the M1 warning is also on `merge2_into`'s doc.
- **Measurement discipline addendum:** this box's single-shot campaign noise is ~±5–8% at 1t and ~±10–26% at 8/32t (untouched code moved that much between campaigns). Effects below that need the interleaved A/B protocol recorded in `v06-G1d-ab.log` (two prebuilt binaries, alternated adjacent in time, paired per-run deltas), not one campaign per build.
- The plan's §5 gates (trotter 32t < 8 ms, rotation_zz 32t < 3.0 ms) were **not met** by any variant. G2 (interleaved `[[u64; W]; 2]` transient key layout) was attempted after G1d and is a **third recorded negative result**: gu2q (its gate cell) +1.9% at 1t — the sort it targeted got *slower*, the SoA comparator having usually decided on the x words alone — and rotation_zz/trotter regressed consistently (merge +9–15% busy: with G1d's borrow the merge compare straddles an SoA a-side and an interleaved b-side, repacking every a-row). Reverted; only worth revisiting jointly with abandoning the G1d borrow, which its own numbers say costs more.

The v0.1 engine is deleted. Differential tests use `test_support::naive_apply_layer` as the oracle instead; a channel that declines preparation (support > `MAX_LOCAL_SUPPORT` qubits, or one that writes outside its declared support) makes `propagate` panic rather than falling back to it. The per-bucket kernels live in `engine/merge.rs`.
...
- `crates/paulistrings/` — pure Rust core (no Python deps). Has `benches/pauli_ops.rs` (criterion). Source modules: `pauli_string`, `pauli_sum`, `accumulator`, `circuit`, `channel/{clifford,rotation,unitary,noise}`, `truncation/{builtin}`, `engine/{bucketed,coset,merge,stats}`. Top-level re-exports in `lib.rs`.
...
- The `phase-timing` Cargo feature (`engine/stats.rs`) gates a per-phase timing breakdown of the propagation engine; it is measurement-only and never in the default feature set, and its non-perturbation of engine output is checked by the existing determinism/fingerprint tests running under the feature in CI (per the determinism policy these are tripwires, not requirements — regenerate/demote freely when an intentional change trips them).

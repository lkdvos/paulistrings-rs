...
*Manual: [Incremental propagation](../../manual/propagation/incremental.md), [Truncation](../../manual/propagation/truncation.md)*
...
The artifact is a **schema-v1 [task file](../../manual/circuits.md#importing-circuits)** — the residual front circuit plus the evolved observable, ready for a QPU-side runner. `task_exact.json` and `task_truncated.json` in the showcase directory are exactly that, and they are the same schema the [cross-engine comparison](../comparisons.md) drives both engines from.
...
The round-trip gap is exactly `0.0`, well inside the `1e-12` bound, and it is a genuine round trip through disk: the evolved observable is [saved to `.npz`](../../manual/operators.md#saving-and-loading) via `paulistrings.io` and read back before being embedded in the task file.
...
Truncation is applied after every *channel*, not after every Python call (see [Truncation](../../manual/propagation/truncation.md)), so splitting a circuit and truncating separately on each half agrees exactly, for any split point, with truncating the full circuit in one shot — a test pins it. The table above is therefore independent of where `k` is chosen. The gap is **not monotone** in the cutoff (2.9e-2 at 3e-2, then 6.3e-2 at 1e-2) — dropped terms carry signs, and a truncated Pauli sum has [no variational bound](../../manual/propagation/truncation.md#no-variational-bound).

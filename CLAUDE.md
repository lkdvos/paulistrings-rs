...
**v0.3 is in progress on this branch** — plan in `research/plans/2026-08-29-v0.3-followups.md`, execution order §1 → §4 → §2 → §3. Landed so far: **§1** (the bucket-count floor is the fixed `DEFAULT_MIN_BUCKETS = 128`, so `B` is a deterministic function of the term count alone) and **§4** (`PauliSum` and `BucketedSum` are merged: there is one bucketed `PauliSum`, canonically ordered by `(bucket index, lex(x, z))`, and a single-bucket sum — any sum of ≤ 1024 terms — is plain lex-sorted, which is why small fixtures see the old order). `from_sum`/`into_sum`/`to_sum`/`propagate_bucketed` no longer exist: `propagate` is the single entry point with no conversion cost, `with_hash` re-partitions, `BuildAccumulator::finalize` picks the hash, iteration is `iter()`/`to_arrays()`/`bucket(b)` (the flat `x()`/`z()`/`coeff()` slice accessors are gone), lookup is `get(x, z)`, and `TruncationPolicy` has a single `finalize_layer`. Also landed: **§2 stage 1** — the engine's parallel unit is one coset of `span(h(D))` (`engine/coset.rs::Gf2Span`; the span, not `h(D)` itself, because a custom channel's delta set need not be XOR-closed), gathered input-major once and merged back **in place** via per-thread scratch (`begin_layer`/`end_layer`/`spare` are gone), with a `u8` delta-index tiebreak in the per-bucket sort that keeps output bitwise-identical to v0.2 and B-independent. The tag was A/B-measured at 5–14% on wide-delta 10⁶ layers and deliberately kept: dropping it provably breaks bitwise B-independence for channels merging ≥3 contributions per key (the B-independence tests now include a `GeneralUnitary2Q` case that catches exactly this). `Channel::support()` now returns a `[u64; W]` bitmask and `PauliRotation` no longer stores a support vector. Known §2 regressions, measured and open: `general_unitary2q` at 10⁶ +56% (32-thread; r = 4 coset working set vs L2), all 10⁴ layer cases +20–26%, and propagation RSS +≈33% on TopN-truncated workloads (fanout-1 workloads are −13%). Still pending: **§3** (`TopN` discards whole tie groups — this one deliberately changes `examples/output/*.csv`).

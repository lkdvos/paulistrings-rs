...
including the two results that are negative. The rigor <em>is</em> the
promotion: a benchmark suite that only reports its wins tells you nothing about
the ones it would have lost.</p>
...
| [**C** Deep Trotter](c-deep-trotter.md) | same circuit, `Z_62`, depth ladder 5/9/15/20 steps, dyadic cutoffs | exact 19-qubit cone at 5 steps; self-converged beyond | **the honest headline is a reachability boundary.** 0.01 accuracy in 0.11 s at 5 steps; at 15–20 steps in the hard interior neither the target nor a reference to score it against is reachable |
| [**D** XXZ chain](d-xxz-chain.md) | Trotterized XXZ chain, `n = 20…100`, free and interacting regimes | statevector at `n ≤ 26`, plus an *analytic* growth law | quadratic term growth confirmed as **exactly `16s²`**; and **the cross-engine ranking changes sign** between 3·10³ and 3·10⁴ terms |
...
## The plateau criterion, and why it is not the obvious one

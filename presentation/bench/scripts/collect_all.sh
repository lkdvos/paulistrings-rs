...
EPS=${EPS:-1.220703125e-4}          # 2^-13: 1.16e6 peak terms
EPS_LARGE=${EPS_LARGE:-1.220703125e-4}
STEPS_LARGE=${STEPS_LARGE:-6}       # 6 Trotter steps at 2^-13: the "large m" arm (see calibration.jsonl)
...
if have calibration; then stage calibration
  ...
  for k in 12 13 14 15 16; do eps=$(python3 -c "print(2**-$k)")
    $B bucketed --terms-only --eps "$eps" | sed 's/"terms_out":\[[^]]*\]/"terms_out":null/' >> "$f"; done
  for k in 12 13 14; do eps=$(python3 -c "print(2**-$k)")
    $B bucketed --terms-only --steps 6 --eps "$eps" | sed 's/"terms_out":\[[^]]*\]/"terms_out":null/' >> "$f"; done
  grep -o '"eps":[^,]*,"steps":[0-9]*,"layers":[0-9]*,"peak_terms":[0-9]*' "$f"
fi
if have ladder; then stage ladder
  f=$D/engine_ladder.jsonl; header "$f"
  $B naive      --threads 1        --reps 5 --eps $EPS --json-out "$f" | grep '^cell'
  $B threadmaps --threads 1,8,32   --reps 3 --eps $EPS --json-out "$f" | grep '^cell'
  $B mergesort  --threads 1,8,32   --reps 3 --eps $EPS --json-out "$f" | grep '^cell'
  $B bucketed   --threads 1,32     --reps 5 --eps $EPS --json-out "$f" | grep '^cell'
  $B bucketed   --threads 32       --reps 5 --eps $EPS $COARSE --json-out "$f" | grep '^cell'
fi
if have scaling; then stage scaling
  f=$D/thread_scaling.jsonl; header "$f"
  T=1,2,4,8,16,32
  $B threadmaps --threads $T --reps 3 --eps $EPS --json-out "$f" | grep '^cell'
  $B mergesort  --threads $T --reps 3 --eps $EPS --json-out "$f" | grep '^cell'
  $B bucketed   --threads $T --reps 5 --eps $EPS --json-out "$f" | grep '^cell'
  $B bucketed   --threads $T --reps 5 --eps $EPS $COARSE --json-out "$f" | grep '^cell'
fi
if have scaling_large; then stage scaling_large
  f=$D/thread_scaling_large.jsonl; header "$f"
  T=1,2,4,8,16,32
  $B bucketed --threads $T --reps 3 --steps $STEPS_LARGE --eps $EPS_LARGE --json-out "$f" | grep '^cell'
  $B bucketed --threads $T --reps 3 --steps $STEPS_LARGE --eps $EPS_LARGE $COARSE --json-out "$f" | grep '^cell'
fi
if have sweep; then stage sweep
  ...
  for cell in "256 128" "1024 128" "4096 128" "16384 64" "65536 16" "262144 16"; do set -- $cell
    $B bucketed --threads 1,16 --reps 5 --eps $EPS --target-bucket-len $1 --min-buckets $2 --json-out "$f" | grep '^cell' | sed "s/^/target=$1 min=$2 /"
  done
fi
if have memory; then stage memory
  f=$D/memory.jsonl; header "$f"
  $B naive      --threads 1  --reps 1 --eps $EPS --json-out "$f" | grep '^cell'
  $B threadmaps --threads 8  --reps 1 --eps $EPS --json-out "$f" | grep '^cell'
  $B mergesort  --threads 8  --reps 1 --eps $EPS --json-out "$f" | grep '^cell'
  $B bucketed   --threads 1  --reps 1 --eps $EPS --json-out "$f" | grep '^cell'
  $B bucketed   --threads 32 --reps 1 --eps $EPS --json-out "$f" | grep '^cell'
fi

...
A device cell (`--device`, feature `cuda`) adds three more, and only there: `device` (a list of the ordinals the cell's partitions ran on, one entry per device: `[0]` for one device, `[0,1,2,3]` for `--device 0,1,2,3`, the rank's own `[d]` on an `--mpi` row), `upload_ns` (`GpuPauliSum::from_host` of the input, including the first-use NVRTC compile) and `download_ns` (`to_host` of the timed call's output); both are outside `wall_ns`.
...
`--format tsv` carries all six as trailing columns, `device` comma-joined and `-1` on a host row.

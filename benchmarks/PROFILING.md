...
- **`--gpu-partitions <n>`** runs the cell as `n` virtual partitions on each device through `GpuPartitionedSum` (`Placement::Devices { devices: [d], per_device: n }`), the partition rows chosen exactly as for a host `--partitions` cell (`--partition-rows`, `--partition-seed`). A remote layer exports through K10 into the pooled `PartnerPayload` columns, exchanges through the in-process transport and merges the received rows in the fused layer at the agreed bucket count; `export_ns` is the K10 pass with its D2H staging, `exchange_ns` the transport call minus the receive body, `chunk_wait_ns` the wait for the whole transfer at the top of that body, `h2d_ns` includes the received rows' upload and `d2h_ns` the exported blocks' download. `PAULISTRINGS_GPU_STAGING=pageable|pinned` selects how an exported block's columns come back to the host (`research/FINDINGS.md`). P=1 (`--device` alone) against `--gpu-partitions 2` is a runtime-knob A/B like the host's: `scripts/ab-compare.sh --probe-b '<args with --gpu-partitions 2>'`, paired on `(layer, threads)`.

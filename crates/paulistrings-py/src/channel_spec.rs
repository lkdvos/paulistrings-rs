...
//! Free-function factories like `gates.h(qubit)` return one; width is fixed when it is appended to a `Circuit`, materializing the monomorphized core channel onto `Circuit<W>`.
...
/// Which single-qubit Pauli a [`ChannelSpec::PauliRotationN`] generator carries at one qubit.
/// Deliberately no `I`: identity positions are expressed by omitting the qubit, so every generator has exactly one spelling.
...
/// An `N x N` complex matrix as the schema's JSON-native form: rows of `[re, im]` pairs.
...
/// What kind of channel `PyChannel` represents. Stored as plain construction parameters so the spec is `Send + Sync` and width-agnostic.
/// `Clone` but not `Copy`: [`ChannelSpec::PauliRotationN`] carries a `Vec`, and `Unitary2Q` alone makes the enum 256+ bytes.
#[derive(Clone, Debug)]
pub enum ChannelSpec {
    ...
    /// `S^dagger`. Its own variant rather than a `Unitary1Q` holding `diag(1, -i)`, so `adjoint()` has a named spelling and the key-preserving Clifford fast path stays instead of routing through a PTM.
    Sdg {
        qubit: u32,
    },
    X {
    ...
    /// A rotation `exp(-i·θ·P/2)` generated by a Pauli string of any weight, held in the compact `(qubit, axis)` form. `Rz`/`Rx`/`Ry` above are the named weight-1 spellings, kept separate so their `Debug` repr does not change.
    /// Invariants established by the constructor (`gates::pauli_rotation_spec`): non-empty, qubit indices pairwise distinct, so `push_into` sets the generator's bit planes directly with no phase to fold.
    PauliRotationN {
        theta: f64,
        paulis: Vec<(u32, Axis)>,
    },
    /// An arbitrary 1-qubit unitary, held as its 2x2 matrix rather than the 4x4 PTM, so the spec stays small; the PTM is computed once at materialization, not per term.
    Unitary1Q {
        qubit: u32,
...
impl ChannelSpec {
    /// The gate name this spec serializes to: task-JSON schema v1's gate vocabulary, also `paulistrings.interop`'s parser vocabulary and the `Circuit` method name that builds it.
    ...
    /// The qubits this spec addresses, in the gate's own argument order — *not* sorted, since order is part of the serialized shape.
    /// The single place the whole vocabulary's qubit fields are enumerated: [`Self::max_qubit`] and [`Self::to_gate_dict`] both read it, so a new variant cannot be bounds-checked and serialized inconsistently.
    ...
    /// The largest qubit index this spec touches, for `CircuitImpl::push_spec`'s bounds check against `num_qubits`. Every variant addresses at least one qubit, so the `unwrap_or` is unreachable.
    pub fn max_qubit(&self) -> u32 {
        self.qubits().into_iter().max().unwrap_or(0)
    }

    /// This spec as one task-JSON schema v1 gate object.
    /// JSON-native throughout, so `json.dumps({"gates": circuit.gates})` needs no custom encoder; a `matrix` is rows of `[re, im]` pairs, what `paulistrings.interop.circuit_from_json` parses back.
    ...
    /// The adjoint channel, or a `ValueError` for a channel with no meaningful one at this API's level.
    /// Unitary gates only: a rotation negates theta, a raw matrix conjugate-transposes, `S`/`S^dagger` swap, the remaining named Cliffords are self-adjoint.
    /// Noise channels are refused by name — their duals are self-adjoint diagonal rescalings that would satisfy `Circuit::adjoint`'s propagation identity without being the circuit's actual inverse.
    ...
    /// Materialize the spec at width `W` and push it onto `circuit`. Qubit indices must already have been checked against `W` — `CircuitImpl::push_spec`'s job, and its only caller.
    pub fn push_into<const W: usize>(&self, circuit: &mut CoreCircuit<W>) {
        match self {
            ...
            // Bit planes set directly rather than by multiplying single-qubit strings together: sites are pairwise distinct by construction, so no `mul_assign` phase can arise.
            Self::PauliRotationN { theta, paulis } => {
            ...
            // Converted to a PTM here, once per circuit push, rather than stored as one in the spec (which would make it 4 KB).
...
/// Opaque, width-erased channel handle exposed to Python. Construct via the free factories `gates.*` and `noise.*`; instantiated at the appropriate width when appended to a `Circuit`.

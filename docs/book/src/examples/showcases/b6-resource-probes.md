...
*Manual: [Cost is terms, not qubits](../../manual/propagation/index.md#cost-model), [Inside the engine](../../manual/propagation/engine.md)*
...
All entropies are in nats, computed in pure Python over the [symplectic arrays](../../manual/operators.md#symplectic-arrays) `PauliSum.x_array()` / `z_array()` / `coefficients_array()` — no core additions.
...
1D kicked-Ising chain, n = 16, 5 Trotter steps, seed `Z_8`, bipartition `[0,8) | [8,16)` — crossing exactly one lattice bond (a chain, not a heavy-hex sublattice, keeps the cut count from confounding the reading). [`policy=None`](../../manual/propagation/settings.md#untruncated-runs) throughout: exact, nothing truncated, no convergence panel owed.

...
| E9 | Observable consistency/convergence | 24 | **real data** | Full 127-qubit canonical task, real genoa allocation (job 7033650): `<O>` at every Trotter step (1-20) for `min_abs_coeff` in {2⁻¹², 2⁻¹⁴, 2⁻¹⁶, 2⁻¹⁸} via `Circuit` slicing (no engine change, `decisions.md` #29) -- one real trajectory line per cutoff, replacing the earlier single-point plot per user request 2026-09-14. Overlaid with the real PauliPropagation.jl reference point (eps=2⁻¹⁶, step 20, `raw/2026-09-14-worker7169-julia`) as a black star -- agrees with the Rust trajectory's own endpoint to `abs_delta=1.67e-16` (`decisions.md` #27), floating-point noise. Julia only computes a final expectation (`runner.jl` has no per-layer expectation, only per-layer term counts), so this is a real endpoint overlay, not a Julia trajectory line. Figure: `figures/real/convergence.png` (renamed from `accuracy.png`, `figures/make_compact_figures.py::make_convergence_figure`'s `julia_points=` param) |

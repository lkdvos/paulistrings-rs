...
- **CUDA backend** — one GPU runs a saturated dense two-qubit layer about 10× faster than a 16-thread host, behind the off-by-default `cuda` feature (`device=` from Python).

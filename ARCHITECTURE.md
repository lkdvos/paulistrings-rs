...
Under NCCL a remote layer sends only the block headers and CSR offsets through `Transport::exchange`, then one `allreduce_sum_u64` go/no-go vote, then, on a unanimous yes, one NCCL group that moves the `x`/`z`/`coeff` columns straight into the receiver's concatenated receive columns, where the receiver computes the fingerprints; this vote is the one call beyond the host layer's, legal only because the mode is group-uniform.
...
The memory consequence is that a partition holds one export volume and one receive volume on its device on top of its sum during a remote layer, where a host partition's equivalent volumes sit in system RAM.

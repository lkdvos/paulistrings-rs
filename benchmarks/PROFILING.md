...
A device cell (`--device`, feature `cuda`) adds four more, and only there: `device` (a list of the ordinals the cell's partitions ran on, one entry per device: `[0]` for one device, `[0,1,2,3]` for `--device 0,1,2,3`, the rank's own `[d]` on an `--mpi` row), `upload_ns` (`GpuPauliSum::from_host` of the input, including the first-use NVRTC compile), `download_ns` (`to_host` of the timed call's output; both are outside `wall_ns`) and `gpu_exchange` (`"host"`, `"device"` or `"nccl"`, the mode the cell actually agreed, from `GpuPartitionedSum::exchange`/`MpiGpuSum::exchange`; `"none"` on a lone unpartitioned device, since there is no exchange to agree).
`--gpu-exchange host|device|nccl` sets the knob the probe agrees, mirroring `PAULISTRINGS_GPU_EXCHANGE`: on an in-process `--gpu-partitions`/multi-`--device` cell it calls `GpuPartitionedSum::set_exchange` after scatter, on an `--mpi --device` cell it sets `PAULISTRINGS_GPU_EXCHANGE` before `MpiGpuSum::scatter_with_rows` reads it (that driver has no exchange setter — see `gpu/rank.rs::start_nccl`); `nccl` needs the `nccl` cargo feature and only ever holds on an `--mpi --device` cell, an in-process cell downgrading it to `device`.
A `--gpu-partitions <n>` cell (`n > 1`) is a device cell and a partitioned cell at once: `partitions` is `n`, `partition_cpus` is the literal `"gpu"`, `pin_memory` is `0`, every partitioned key above is written (`export_ns`, `exchange_ns`, `rows_exported`, `bytes_exported`, the per-partition lists), `upload_ns` is the scatter (filter and upload of every partition) and `download_ns` the gather.
An `--mpi --device` row is a distributed cell and a device cell at once: `rank`/`ranks` as above, `partitions` `1`, `partition_cpus` `"gpu"`, `upload_ns` this rank's scatter and `download_ns` the collective gather.
`--format tsv` carries all seven as trailing columns, `device` comma-joined, `-1` on a host row and `gpu_exchange` `"none"` there.
...
- **`--threads` is not swept**: one cell per layer, `--threads[0]` echoed into the row. `--partitions` must stay at `1` (device partitions come from `--gpu-partitions`), `topn:<N>` is refused, `--occupancy-at` is unsupported, and `rotation_remote` needs more than one device partition (several ordinals, or `--gpu-partitions > 1`), or `--mpi` at more than one rank.
- **`--device <csv|auto>`** takes a list: several ordinals (or `auto`, every visible device) run the cell through `GpuPartitionedSum` with `Placement::Devices { devices, per_device: <--gpu-partitions> }`, one partition per device by default. Under `--mpi` it takes one ordinal or `auto`, the rank's node-local device (`gpu::local_device_for_rank`), and each rank runs its partition through `MpiGpuSum`.
- **`--gpu-partitions <n>`** runs the cell as `n` virtual partitions on each device through `GpuPartitionedSum` (`Placement::Devices { devices: [d], per_device: n }`), the partition rows chosen exactly as for a host `--partitions` cell (`--partition-rows`, `--partition-seed`). A remote layer exports through K10, exchanges through the in-process transport and merges the received rows in the fused layer at the agreed bucket count, in one of two payload forms (`PAULISTRINGS_GPU_EXCHANGE=device|host`, default `device`, `research/FINDINGS.md`). Under `device` the blocks stay in device memory, fingerprints included: `export_ns` is K10 plus the fingerprint pass on the sender, `exchange_ns` the whole transport call including the receiver's device-to-device copies into its `recv_*` columns, and `h2d_ns`/`d2h_ns` carry only the CSR offsets (kilobytes). Under `host` the blocks stage through the pooled `PartnerPayload` columns: `export_ns` is K10 with its D2H staging, `exchange_ns` the transport call minus the receive body, `h2d_ns` includes the received rows' upload and `d2h_ns` the exported blocks' download, and `PAULISTRINGS_GPU_STAGING=pageable|pinned` selects how an exported block's columns come back to the host. `chunk_wait_ns` is the wait for the whole transfer at the top of the receive body in both forms. A device sender merges each partner's rows by key before the exchange (ARCHITECTURE.md §Partitioning), so `rows_exported`/`bytes_exported` count the merged rows and `export_ns` includes the merge; `PAULISTRINGS_GPU_PREMERGE=off` restores the unmerged export as a runtime-knob A/B. P=1 (`--device` alone) against `--gpu-partitions 2` is a runtime-knob A/B like the host's: `scripts/ab-compare.sh --probe-b '<args with --gpu-partitions 2>'`, paired on `(layer, threads)`.

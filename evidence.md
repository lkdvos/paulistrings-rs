...
| E8 | Communication-aware hash | 28/28b | tooling-ready | Schema/driver/normalize/figure stack built and tested 2026-09-13 (`decisions.md` #21): `partition_row_policy` field, `run_cell.py`'s `"cut"` (BFS locality heuristic) vs `"random"` policies, `normalize.hash_communication`, `make_hash_communication_figure`. Real local proof (32 qubits, `partitions=2`, in-process engine, no MPI, preflight monkeypatched since this login host isn't genoa): random exported 8975 rows/294120 bytes vs. cut's 100 rows/3584 bytes for the same cell — not schema-legitimate campaign data (preflight was bypassed, not passed), and not yet run at cluster (127-qubit) scale. `jobs/campaign-genoa-e8.sbatch` prepared 2026-09-14 (`campaign.json` C5_e8_single_node) to get a real, hardware-valid pair; not yet submitted. The distributed (`comm=`) path only supports `"random"` so far |

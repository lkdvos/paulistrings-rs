...
*Manual: [Operators](../manual/operators.md), [Engine and propagation](../manual/propagation/index.md)*
...
The `[row, 0]` index picks word 0 of the term, which is the whole key here because 4 qubits fit in one 64-bit word — see [Symplectic arrays](../manual/operators.md#symplectic-arrays) for the general layout.
...
A flat answer over a *changing* sum is not evidence of convergence, and on a real problem that is exactly the trap to check for — [Validating a result](../manual/propagation/validation.md) is the full recipe.

## Where to go next

- [Manual](../manual/index.md) — operators, circuits, engine and propagation, measurements, read start to finish or by topic.
- [Validating a result](../manual/propagation/validation.md) — the cutoff sweep, the retained norm, and when a number may be quoted.
- [Direction](../manual/propagation/direction.md) and [Direction semantics](../library/direction.md) — what `"heisenberg"` and `"forward"` compute, and which state `expectation` is then read against.
- [Measurements](../manual/measurements.md) — reading a sum against a stabilizer state or another sum, not just a product state.
- [Incremental propagation](../manual/propagation/incremental.md) — the same walkthrough, stepped out into a Trotterized time series.
- [Against other tools](comparisons.md) — whether Pauli propagation is the right method for your problem at all.
